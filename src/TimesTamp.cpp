...
TimesTamp::TimesTamp() : m_secSinceEpoch(time(nullptr)) {}

TimesTamp::TimesTamp(time_t secSinceEpoch) : m_secSinceEpoch(secSinceEpoch) {}
...
std::string TimesTamp::tostring() const // 返回时间戳对象表示的字符串形式的时间（yyyy-mm-dd HH:MM::SS）
{
    ...
    snprintf(buf, sizeof(buf), "%4d-%02d-%02d %02d:%02d:%02d", t->tm_year + 1900, t->tm_mon + 1, t->tm_mday, t->tm_hour,
             t->tm_min, t->tm_sec);
...
std::string TimesTamp::tostringData() const // 返回时间戳对象表示的字符串形式的时间（yyyy-mm-dd）
{
    ...
    char buf[128] = {0};
    // 使用 snprintf 进行格式化，它会自动处理+1900和+1，并且能方便地控制格式
    snprintf(buf, sizeof(buf), "%4d-%02d-%02d", t->tm_year + 1900, t->tm_mon + 1, t->tm_mday);

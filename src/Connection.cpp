...
Connection::Connection(std::shared_ptr<Socket> psocket, EventLoop* ploop) :
    m_psocket(psocket), m_ploop(ploop), m_pchannel(std::make_shared<Channel>(ploop, m_psocket)), m_disconnect(false)
{
    // 设置 和通信套接字关联的Channel对象 的读事件回调函数
    m_pchannel->setreadeventcb([this]() { this->onmessage(); });

    // 设置 关闭TCP连接时，析构该条TCP连接通信套接字所属的Connection对象 的回调函数
    m_pchannel->setcloseconnectioncb([this]() { closeconnection(); });

    // 设置 和通信套接字关联的Channel对象 的写事件回调函数
    m_pchannel->setwriteeventcb([this]() { this->sendto(); });
...
// 获取 通信套接字fd
int Connection::fd() const { return m_psocket->fd(); }
...
// 设置 m_sendcompletecb
void Connection::setsendcomplete(std::function<void(std::shared_ptr<Connection>)> func) { m_sendcompletecb = func; }
...
void Connection::onmessage()
{
    ...
    //     len = ntohl(len);
    //     if (m_inputbuf.size() < len + sizeof(len)) // m_inputbuf
    //     里面不是一条完整的数据，暂时存放在读缓冲区内，等下次客户端发送余下的数据。
    //     {
    //         break;
    //     }
...
void Connection::addToEpoll() { m_pchannel->enablereading(); }

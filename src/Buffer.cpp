...
Buffer::Buffer(size_t initialSize) :
    m_buffer(initialSize + kCheapPrepend), m_readerIndex(kCheapPrepend), m_writerIndex(kCheapPrepend)
{
...
// 获取当前能写的数据量
size_t Buffer::writableBytes() const { return m_buffer.size() - m_writerIndex; }
...
// 获取当前可读数据的首地址
const char* Buffer::peek() const { return begin() + m_readerIndex; }
...
// 获取写下标的位置
const char* Buffer::beginWrite() const { return begin() + m_writerIndex; }

// 使缓冲区可以放得下len字节的数据。给缓冲区扩容或者移动m_readerIndex位置
void Buffer::makeSpace(size_t len)
{
    if (writableBytes() + prependableBytes() <
        len + kCheapPrepend) // m_buffer所有空闲空间都不足以放下长度为len字节的数据
    {

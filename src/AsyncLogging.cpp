...
AsyncLogging::AsyncLogging(uint32_t flushInterval) :
    m_output(&std::cerr), m_flushInterval(flushInterval), m_currentBuffer(new LogBuffer), m_nextBuffer(new LogBuffer),
    m_running(false)
{
    m_nextBuffer->reset();
...
void AsyncLogging::ThreadFunc()
{
    ...
    while (m_running || m_currentBuffer->cur > 0) {
        /////////////////////////////锁区域/////////////////////////////////////////
        {
            ...
            m_buffers.push_back(
                std::move(m_currentBuffer)); // 将当前缓冲区m_currentBuffer里的内容添加到缓冲队列m_buffers里
            m_currentBuffer =
                std::move(newBuffer1); // 用 newBuffer1 缓冲区替换 m_currentBuffer，保证日志系统的前端不会停
        ...
        if (buffersToWrite.size() >
            25) // 如果日志系统后端的缓冲队列 bufferToWrite 里面日志块的数量大于25，则丢弃多余的日志块，只剩下两块。
        {
            char msg[256] = {0};
...
void AsyncLogging::setOutput(std::ostream* os) { m_output = os; }

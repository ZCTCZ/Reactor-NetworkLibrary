...
private:
    ...
    std::unordered_map<int, std::list<std::weak_ptr<Connection>>::iterator>
        m_connectionmap; // 从 fd 快速定位到 list 中的节点
    ...
    std::function<void(int)>
        m_timerCallback; // 回调函数，定时器触发后执行，调用cpServer类的removeTimeOutConnection函数，用来删除TcpServer管理的Connection连接
    std::function<void(int)>
        m_delayDeleteCallback; // 回调函数，每轮事件循环后执行，调用TcpServer类的deleteconnection函数，用来删除TcpServer管理的Connection连接
};

...
/*
    ...
    %S: 秒数（00-59）
*/
Log::Log(Level lv, const char* file, int line, const char* func) : lv_(lv)
{
    ...
    os_ << '[' << level2str(lv) << ']' << "[PID:" << syscall(SYS_gettid) << ']' // 线程ID
        << '[' << std::put_time(&tm, "%Y-%m-%d %H:%M:%S") << ']' << '[' << file << ':' << line << ']' << '[' << func
        << "] ";
}
...
Log::~Log()
{
    if (AsyncLogging::exist) // 如果定义了 AsyncLogging 类的单例对象，就可以使用异步日志输出
    {
        ...
        asyncLog->setOutput(&GetOutputStream());
...
void Log::SetOutputTarget(OutputTarget target, const std::string& filename)
{
    ...
    // 如果开启了异步日志，设置日志异步输出的目的地
    if (AsyncLogging::exist) {
        AsyncLogging::getInstance()->setOutput(&GetOutputStream());
    }
}
